...
class AppContainer extends Component {
  ...
  fetchPlaces = (mapProps, map) => {
    ...
    // const storePlaceIds = this.storePlaceId;
    // let getAllResults;
    ...
    function getPlaces(myResults, status, pagination) {
      if (status === "OK") {
        ...
        myResults.forEach(res => {
          return newArr.push(res.place_id);
        });
      }
    }
    const placesResults = newArr;
    console.log("Get info function results 💹", placesResults);
    function storePlaceIds(...places) {
      console.log("This is google 🎷", google);

      console.log("First Initial Results All 📌", places);

      for (let i = 0; i < places.length; i++) {
        console.log("for loop", places[i]);
      }
    }

    storePlaceIds(placesResults);
    // const interval = 900;
    ...
    //       if (status === "OK") {
    //         detailsArr.push(results);
    //       }
    //     }
    //   }, interval);
    //   return detailsArr;
    // });
    // console.log("These are my places 🎷", detailsArr);
  };
